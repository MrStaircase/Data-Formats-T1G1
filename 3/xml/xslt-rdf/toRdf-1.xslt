...
# Data - Customers
<xsl:for-each select="//s:Visitor">
  <xsl:variable name="visitorIRI" select="@iri"/>
  <xsl:variable name="firstWithIRI" select="//s:Visitor[@iri = $visitorIRI][1]"/>
  <xsl:variable name="thisNode" select="."/>
  <xsl:apply-templates select=".[count($thisNode | $firstWithIRI) = count($firstWithIRI)]" mode="customer"/>
</xsl:for-each>
# Data - Employees
...
<xsl:for-each select="//s:ProductRef">
  <xsl:variable name="prodIRI" select="@iri"/>
  <xsl:variable name="firstRefForProd" select="//s:ProductRef[@iri = $prodIRI][1]"/>
  <xsl:variable name="thisRef" select="."/>
  <xsl:variable name="prodNode" select="//s:Product[@iri = $prodIRI][1]"/>
  <xsl:apply-templates
      select="$prodNode[count($thisRef | $firstRefForProd) = count($firstRefForProd)]"
      mode="prod-manuf">
...
<xsl:text>
    ...
    <xsl:template match="s:Phone_number" mode="phone-start"/>
